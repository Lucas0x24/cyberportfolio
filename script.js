...
window.addEventListener('scroll', () => {
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', function(e) {
            ...
            const mailtoLink = `mailto:lucashermiz@gmail.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(emailBody)}`;
            
            // Show success message
            showNotification('Opening email client... Please send the email manually.', 'success');
...
document.querySelectorAll('.skill-category').forEach(category => {
    category.addEventListener('mouseenter', function() {
        ...
        this.style.boxShadow = '0 12px 40px rgba(0, 212, 255, 0.3)';
    });
...
document.querySelectorAll('.btn, .project-card, .skill-category, .cert-card').forEach(element => {
    element.style.transition = 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)';
});

console.log('Cybersecurity Portfolio Website Loaded Successfully! 🛡️');
